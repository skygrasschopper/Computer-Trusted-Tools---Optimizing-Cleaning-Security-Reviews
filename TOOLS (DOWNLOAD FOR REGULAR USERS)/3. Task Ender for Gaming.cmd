::Intro
@echo off
Title Task Ender for Gaming (Feel free to close this tool when it starts cleaning junk files!)
MODE CON: COLS=90 LINES=50 & color 0c
::Intro
echo RUN AS ADMIN! Feel free to close this tool when it starts cleaning junk files, because you only need to end background tasks, assuming you don't have that many junk files...
echo.
color 0a
echo Steps:
...
echo 6. Cache (#Caution! Check your tools, like do you use inetpub?)
echo.
echo Please edit the script according to your needs. Especially paths with #Caution!
echo Keeping it organized will help a lot.
echo.
echo Rebooting twice after using is recommended, to let CACHE regenerate and prevent errors,
echo However the author has made sure they will and prevented deleting most cache (you can enable them)
echo Some Cache store your personal settings that's why the author don't want to mess with them.
echo Enabling some has crashed 20 Virtual Machines and 5 Contributers' computers
echo.
echo Made for
hostname
echo by Skygrass Chopper
echo Version 4.00 (Stable with #Caution!) 
date /t 
echo (Require user to manually check file paths, espescially CACHE, which might store personalization settings)
echo (Most cache that has been disabled are safe to enable for general users)
timeout /t 30
::1) #End-Tasks
color 4F
...
echo    ***************************************
    taskkill.exe /F /FI "USERNAME eq %USERNAME%" /FI "IMAGENAME ne ClassicShellService.exe" /FI "IMAGENAME ne explorer.exe" /FI "IMAGENAME ne dwm.exe" /FI "IMAGENAME ne cmd.exe" /FI "IMAGENAME ne mbam.exe" /FI "IMAGENAME ne teamviewer.exe" /FI "IMAGENAME ne TeamViewer_Service.exe" /FI "IMAGENAME ne Taskmgr.exe" /FI "IMAGENAME ne Teamviewer_Desktop.exe" /FI "IMAGENAME ne MsMpEng.exe" /FI "IMAGENAME ne tv_w32.exe" /FI "IMAGENAME ne LogMeIn.exe" /FI "IMAGENAME ne Tron.bat" /FI "IMAGENAME ne rkill.exe" /FI "IMAGENAME ne rkill64.exe" /FI "IMAGENAME ne rkill.com" /FI "IMAGENAME ne rkill64.com" /FI "IMAGENAME ne conhost.exe" /FI "IMAGENAME ne dashost.exe" /FI "IMAGENAME ne wget.exe" /FI "IMAGENAME ne TechToolbox.exe" /FI "IMAGENAME ne vmtoolsd.exe" /FI "IMAGENAME ne conhost.exe" /FI "IMAGENAME ne 7z.exe" /FI "IMAGENAME ne courgette.exe" /FI "IMAGENAME ne GeForce_Experience_Permission.exe" /FI "IMAGENAME ne NVIDIA_GeForce_Experience.exe" /FI "IMAGENAME ne NVIDIA_Notification.exe" /FI "IMAGENAME ne NVIDIA_Share.exe" /FI "IMAGENAME ne nvcontainer.exe" /FI "IMAGENAME ne NVDisplay.Container.exe" /FI "IMAGENAME ne nvgwls.exe" /FI "IMAGENAME ne nvsphelper64.exe" /FI "IMAGENAME ne NVIDIA_Web_Helper.exe" /FI "IMAGENAME ne nvnodejslauncher.exe" /FI "IMAGENAME ne Nvidia*" /FI "IMAGENAME ne NVIDIA*" /FI "IMAGENAME ne GeForce*" /FI "IMAGENAME ne nvcplui.exe" /FI "IMAGENAME ne NvGpuUtilization.exe" /FI "IMAGENAME ne NvStereoUtilityOGL.exe" /FI "IMAGENAME ne NvStTest.exe" /FI "IMAGENAME ne NvStView.exe" /FI "IMAGENAME ne NvStWiz.exe" /FI "SERVICES ne NVDisplay.ContainerLocalSystem" /FI "SERVICES ne NVIDIA_LocalSystem_Container" /FI "SERVICES ne NVIDIA*" /FI "SERVICES ne GeForce*"
cls
color 06

::Start Service
Title 1.1) Start Service
start powershell.exe -Command "Start-Service -Name {NVDisplay.ContainerLocalSystem}"
start powershell.exe -Command "Start-Service -Name {NvContainerLocalSystem}"
::Start Service 2nd way (more reliable)
Title 1.2) Start Service (more reliable)
start powershell.exe -Command "Start-Service -DisplayName {NVIDIA Display Container LS}"
start powershell.exe -Command "Start-Service -DisplayName {NVIDIA LocalSystem Container}"
::Restart Apps
Title 1.3) Restart Apps
start cmd.exe @cmd /k "MODE CON: COLS=20 LINES=20 & "C:\Program Files\NVIDIA Corporation\NVIDIA GeForce Experience\NVIDIA GeForce Experience.exe"& exit"
start cmd.exe @cmd /k "MODE CON: COLS=20 LINES=20 & "C:\Program Files\NVIDIA Corporation\NVIDIA GeForce Experience\GeForce Experience Permission.exe" & exit"
start cmd.exe @cmd /k "MODE CON: COLS=20 LINES=20 & "C:\Program Files\NVIDIA Corporation\NVIDIA GeForce Experience\NVIDIA Notification.exe" & exit"
start cmd.exe @cmd /k "MODE CON: COLS=20 LINES=20 & "C:\Program Files\NVIDIA Corporation\NVIDIA GeForce Experience\NVIDIA Share.exe" & exit"
start cmd.exe @cmd /k "MODE CON: COLS=20 LINES=20 & "C:\Program Files\NVIDIA Corporation\NvContainer\nvcontainer.exe" & exit"
start cmd.exe @cmd /k "MODE CON: COLS=20 LINES=20 & "C:\Windows\System32\DriverStore\FileRepository\nvlti.inf_amd64_af22935532babf10\Display.NvContainer\NVDisplay.Container.exe" & exit"
start cmd.exe @cmd /k "MODE CON: COLS=20 LINES=20 & "C:\Program Files (x86)\NVIDIA Corporation\NvNode\NVIDIA Web Helper.exe" & exit"
start cmd.exe @cmd /k "MODE CON: COLS=20 LINES=20 & "C:\Program Files (x86)\NVIDIA Corporation\NvNode\nvnodejslauncher.exe" & exit"
start cmd.exe @cmd /k "MODE CON: COLS=20 LINES=20 & "%userprofile%\AppData\Local\NVIDIA\NvBackend\ApplicationOntology\NvOAWrapperCache.exe" & exit"
start cmd.exe @cmd /k "MODE CON: COLS=20 LINES=20 & "%userprofile%\AppData\Local\NVIDIA\NvBackend\ApplicationOntology\OAWrapper.exe" & exit"
start cmd.exe @cmd /k "MODE CON: COLS=20 LINES=20 & "C:\Program Files\WindowsApps\NVIDIACorp.NVIDIAControlPanel_8.1.961.0_x64__56jybvy8sckqj\nvcplui.exe" & exit"
start cmd.exe @cmd /k "MODE CON: COLS=20 LINES=20 & "C:\Program Files\WindowsApps\NVIDIACorp.NVIDIAControlPanel_8.1.961.0_x64__56jybvy8sckqj\NvGpuUtilization.exe" & exit"
start cmd.exe @cmd /k "MODE CON: COLS=20 LINES=20 & "C:\Program Files\WindowsApps\NVIDIACorp.NVIDIAControlPanel_8.1.961.0_x64__56jybvy8sckqj\NvStereoUtilityOGL.exe" & exit"
start cmd.exe @cmd /k "MODE CON: COLS=20 LINES=20 & "C:\Program Files\WindowsApps\NVIDIACorp.NVIDIAControlPanel_8.1.961.0_x64__56jybvy8sckqj\NvStTest.exe" & exit"
start cmd.exe @cmd /k "MODE CON: COLS=20 LINES=20 & "C:\Program Files\WindowsApps\NVIDIACorp.NVIDIAControlPanel_8.1.961.0_x64__56jybvy8sckqj\NvStView.exe" & exit"
start cmd.exe @cmd /k "MODE CON: COLS=20 LINES=20 & "C:\Program Files\WindowsApps\NVIDIACorp.NVIDIAControlPanel_8.1.961.0_x64__56jybvy8sckqj\NvStWiz.exe" & exit"

::2) #Network-Flush
color 0a
Title 2) #Network-Flush (CLOSE THIS TOOL AND START GAMING)
...
::3) #Disk-Cleanup
color 06
Title 3) #Disk-Cleanup (CLOSE THIS TOOL AND START GAMING)
MODE CON: COLS=50 LINES=50
echo    ======================================
echo    ==  Move your mouse in CIRCLE now!  ==
echo    == This is to prevent getting stuck ==
echo    ======================================
    start cmd.exe @cmd /k "Mode CON: COLS=20 LINES=20 & color 03 & Title 2) Disk Cleanup & echo    ====================================== & echo    ==  Move your mouse in CIRCLE now!  == & echo    == This is to prevent getting stuck == & echo    ====================================== & cleanmgr /verylowdisk & color 06 & exit"
color 0a

::4) #!Disable-space-wasting-Features    (#Caution! - Do you want Hibernate feature on? It uses 10GB)
color 03
Title 4) #Disable-space-wasting Features   (#Caution!) (CLOSE THIS TOOL AND START GAMING)
...
color 0B
Title 5) #Reveal-Hidden-Files (CLOSE THIS TOOL AND START GAMING)
MODE CON: COLS=90 LINES=50
echo    ============================
...
Title Cleaning
    ::6) #Temp  (Safe)
    Title 6) #Temp (CLOSE THIS TOOL AND START GAMING)
        ::Windows-Temp
        Title 6a) Windows-Temp (CLOSE THIS TOOL AND START GAMING)
            ::Win-Temp
            Title 6a.1) Win-Temp
            ...
            ::Win-Logs
            color 01
            Title 6a.2) Win-Logs (CLOSE THIS TOOL AND START GAMING)
                ...
                color 02
                Title 6a.2.1) Win-Logs-Custom-Scripts (CLOSE THIS TOOL AND START GAMING)
                    ::Deletes Event Logs
                    start cmd.exe @cmd /k "for /F "tokens=*" %1 in ('wevtutil.exe el') DO wevtutil.exe cl "%1""
                    ::start powershell.exe -Command "Get-EventLog -LogName * | ForEach { Clear-EventLog $_.Log }; Set-Location “C:\Windows\Temp”; Remove-Item * -recurse -force; Set-Location “C:\Windows\Prefetch”; Remove-Item * -recurse -force; Set-Location “C:\Documents and Settings”; Remove-Item “.\*\Local Settings\temp\*” -recurse -force; Set-Location “C:\Users”; Remove-Item “.\*\Appdata\Local\Temp\*” -recurse -force; exit"
                    ::start powershell.exe -Command "Get-EventLog -LogName * | ForEach { Clear-EventLog $_.Log }; Set-Location “C:\Windows\Temp”; Remove-Item * -recurse -force; Set-Location “C:\Windows\Prefetch”; Remove-Item * -recurse -force; Set-Location “C:\Users”; Remove-Item “.*\Appdata\Local\Temp*” -recurse -force; exit"

            ::Win-Dumps     (Crash Dump Reports)
            color 0E
            Title 6a.3) Win-Dumps (CLOSE THIS TOOL AND START GAMING)
            ...
            ::Win-Diagnosis
            color 06
            Title 6a.4) Win-Diagnosis (CLOSE THIS TOOL AND START GAMING)
            ...
            ::Win-History
            color 09
            Title 6a.5) Win-History (CLOSE THIS TOOL AND START GAMING)
            ...
            color 01
            Title 6a.6) Win-Folder (CLOSE THIS TOOL AND START GAMING)
        ...
        ::Apps-Temp
            ::App-Temp
            color 0E
            Title 6b.1) App-Temp (CLOSE THIS TOOL AND START GAMING)
                del /q/f/s %temp%
                del /s /f /q %Temp%\*.*
            ...
            ::App-Logs
            color 0D
            Title 6b.2) App-Logs (CLOSE THIS TOOL AND START GAMING)
            ...
            ::App-Dumps
            color 06
            TItle 6b.3) App-Dumps (CLOSE THIS TOOL AND START GAMING)
            ...
            ::App-Report
            color 0B
            Title 6b.4) App-Report (CLOSE THIS TOOL AND START GAMING)
        ...
        Title 7) Driver-Temp (CLOSE THIS TOOL AND START GAMING)
        color 0C
    ...
    ::#Caution!
    color 06
    Title 8) #Caution Files! (CLOSE THIS TOOL AND START GAMING)
        ...
        Title 8) #!Cache (Caution Files) (CLOSE THIS TOOL AND START GAMING)
            ::Windows-Cache     (#!Caution, some may store user login data and preferences!)
            Title 8a) Windows-Cache (CLOSE THIS TOOL AND START GAMING)
                ::Win-Cache     (Disk Cleanup already deleted Cache that are safe to delete!)
                color
                Title 8a.1) Win-Cache (CLOSE THIS TOOL AND START GAMING)
                    ...
                    ::del /q/f/s "C:\Windows\ServiceProfiles\NetworkService\AppData\LocalLow\Microsoft\CryptnetUrlCache"
                        ...
                        color 03
                        Title #8a.2) Reset Microsoft Store Cache (CLOSE THIS TOOL AND START GAMING)
                        start cmd.exe @cmd /k "MODE CON: COLS=20 LINES=20 & Title 8a.2) Reset Microsoft Store Cache & color 0a & wsreset & exit"


                ::Win-Prefetch  (100% Safe)
                color 0c
                Title 8a.3) Win-Prefetch (CLOSE THIS TOOL AND START GAMING)
        ...
        color 0B   
        Title 8b.1) App-Cache (CLOSE THIS TOOL AND START GAMING)
    ...
    color 03
    Title 9) #!Temp (#Caution files) (CLOSE THIS TOOL AND START GAMING)
            ::!Windows-Temp
            ::!Win-Temp
            Title 9a.1) !Win-Temp (CLOSE THIS TOOL AND START GAMING)
            ...
            ::!Win-Logs
            color 09
            Title 9a.2) !Win-Logs (CLOSE THIS TOOL AND START GAMING)
                ...
                del /q/f/s "C:\inetpub\history"
                    ...
                    del /q/f/s "%userprofile%\Local Settings\Temp\Cookies"
                        ::custom strong script to delete index.dat
                            ...
                            ::start cmd.exe @cmd /k "MODE CON: COLS=20 LINES=20 & Title Delete index.dat & echo Index.dat are files hidden on your computer that contain all of the Web sites that you have ever visited... & color 0b & cd C:\ & del index.dat /s & exit"
        ...
        ::!Apps
        color 01
        Title 9b) !Apps (CLOSE THIS TOOL AND START GAMING)
            ::!App-Logs
            Title 9b.1)!App-logs
    ...
    color 05
    Title 10) #!Page-File   (#Caution)  (#Stability-issues) (CLOSE THIS TOOL AND START GAMING)
    ...
    Title 11) SKY-Tool-Delete (CLOSE THIS TOOL AND START GAMING)
        ::Default Windows Reports
        Title 11.1) Default Windows Reports (CLOSE THIS TOOL AND START GAMING)
        ...
        Title 11.2.1) Custom Reports (CLOSE THIS TOOL AND START GAMING)
...
::12) OTHER CUSTOM SCRIPT
    Title 12) Other custom scripts (CLOSE THIS TOOL AND START GAMING)
    ::TRON-AUTHORS
    Title 12.1) TRON-AUTHORS (CLOSE THIS TOOL AND START GAMING)
    color 12
...
::#Optimize-Speed
    ::12) Defragmentation
    color 0B
    Title 12) Defragmentation & Optimization (CLOSE THIS TOOL AND START GAMING)
    vol
...
::#System-Health
color 0B
Title 13) System-Health (CLOSE THIS TOOL AND START GAMING)
    ...
    Title 13.1a) #Windows-Regenerate
        ...
        color 0D 
        Title 13.1b) App-Regenerate (CLOSE THIS TOOL AND START GAMING)

    ::#Health-Check
    color 0a
        ::Update
        Title 13.2) Update (CLOSE THIS TOOL AND START GAMING)
            ::Windows-Update
            Title 13.2a) Windows update (CLOSE THIS TOOL AND START GAMING)
        ...
        ::Verify
        color 0D
        Title 14) Verify System & Windows (CLOSE THIS TOOL AND START GAMING)
            ::Verify System
            Title 14.1) Verify System
                sfc /scannow
            ::Verify Windows
            color 03
            Title 14.2) Verify Windows (CLOSE THIS TOOL AND START GAMING)
        ...
        echo   ###########################################################
        ::Boot sector scan
        Title 15.1) Boot sector scan (CLOSE THIS TOOL AND START GAMING)
        ...
        ::Disk Check on Boot
        color 06
        Title 16) Disk Check on boot? (CLOSE THIS TOOL AND START GAMING)
        chkdsk /r

::17) Clear Event Viewer Log
Title 17) Clear Event Viewer Log (CLOSE THIS TOOL AND START GAMING)
