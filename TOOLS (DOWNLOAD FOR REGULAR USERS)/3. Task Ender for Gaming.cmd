...
MODE CON: COLS=143 LINES=51 & color 09
::Intro
echo                                                                                 RUN AS ADMIN!                                                                                      
echo Steps:                                                                                /\_/\
echo 1. Auto end tasks (#Caution! DON'T CLOSE TOOL HERE) 2. Disk Cleanup 3. Network Flush ((-v-))                   
echo 4. Disable space wasting features (#Caution! See if you like using them)             ():::()                      ;'-. 
echo 5. Reveal Hidden Files 6. Clear Temp, logs, Crashdumps,                               VV-VV           `;-._        )  '---.._  
echo System Event logs, Diagnosis logs, Reports, History of virus scanner etc                                \  `-.__.-'          `'._
echo 7. Cache (#Caution! Check your tools, like do you use inetpub?)                                        /_.-'-._         _,   ' ---)         
echo (YOU HAVE BEEN WARNED, do not edit and enable commands that were disabled!)                            `       `'------/_.'----``
echo                                                                                                                        `
date /t
...
::Taskkill finished
Title Start gaming!
msg * All unecessary tasks have beek closed and you can start gaming! Feel free to close the tool unless you want to continue optimizing your PC. ~Skygrass Chopper
...
Title Meanwhile closing restarted Nvidia apps
taskkill.exe /IM "NVIDIA GeForce Experience.exe" /F
taskkill.exe /IM "nvcplui.exe" /F
        ...
        Title 6a) Windows-Temp (CLOSE THIS TOOL AND START GAMING)
            ...
            Title 6a.2) Win-Logs (CLOSE THIS TOOL AND START GAMING)
                ...
                Title 6a.2.1) Win-Logs-Custom-Scripts (CLOSE THIS TOOL AND START GAMING)
                    ...
                    ::(BREAKS)start powershell.exe -Command "Get-EventLog -LogName * | ForEach { Clear-EventLog $_.Log }; Set-Location “C:\Windows\Temp”; Remove-Item * -recurse -force; Set-Location “C:\Windows\Prefetch”; Remove-Item * -recurse -force; Set-Location “C:\Users”; Remove-Item “.*\Appdata\Local\Temp*” -recurse -force; exit"
                    ::(Crash)taskkill /IM svchost.exe /F
                    start powershell.exe -Command "wevtutil el | Foreach-Object {wevtutil cl "$_"}"
                    start powershell.exe -Command "Get-EventLog -LogName * | ForEach { Clear-EventLog $_.Log }"
